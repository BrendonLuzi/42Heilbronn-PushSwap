...
/*   Updated: 2023/02/08 13:52:32 by bluzi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*make_line(char **remainder, int nl_pos, int rem_len)
{
	...
	if (!nl_pos || nl_pos == rem_len)
	{
		...
		ft_strlcpy(line, *remainder, rem_len + 1);
		free(*remainder);
		*remainder = NULL;
	}
	else
	{
		...
		ft_strlcpy(line, *remainder, nl_pos + 1);
		ft_strlcpy(buf, *remainder + nl_pos, rem_len - nl_pos + 1);
		free(*remainder);
		*remainder = buf;
	}
	return (line);
	...
	while (r && *remainder)
	{
		...
		if (ft_find_nl(*remainder) || (r == 0 && (**remainder)))
		{
			line = make_line(remainder,
					ft_find_nl(*remainder), ft_strlen(*remainder));
			return (line);
		}
